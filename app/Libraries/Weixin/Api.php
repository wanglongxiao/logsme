...
use App\Libraries\Weixin\Utils\HttpCurl;
use App\Libraries\Weixin\Utils\Error;
use App\Libraries\Weixin\Utils\SHA1;
...
class Api 
{
    ...
    /**
     * Added by AlexWang, 20151003
     * 
     * upload a tmp mpnews and get the mediaid
     * 
     */
    public function uploadNews($articles)
    {
    	$url = self::API_DOMAIN . 'cgi-bin/media/uploadnews?access_token=' . $this->get_access_token();
    	$articles1 = array();
     ...
     * send preview msg
     */
    public function sendPreview ($mediaid, $touseropenid)
    {
    	$mpnews = json_encode(array('media_id' => $mediaid));
    	$xml = sprintf('{
    			"touser" : "%s",
    			"mpnews" : %s,
    			"msgtype" : "%s"}',
    			...
    			$mpnews,
    			"mpnews");
    	$url = self::API_DOMAIN . 'cgi-bin/message/mass/preview?access_token=' . $this->get_access_token();
    	$res = HttpCurl::post($url, $xml, 'json');
    	// 异常处理: 获取时网络错误
    	if ($res === false) {
    		return Error::code('ERR_GET');
    ...
    /**
     * Added by AlexWang, 20151004
     *
     * send msg to all users of a group
     * create a "test group" , make testing easy
     */
    public function sendMsgToGroup ($mediaid, $groupid = "all")
    {
    	if ($groupid == 'all') {
    		$filter = json_encode(array('is_to_all' => true));
    	} else {
    		$filter = json_encode(array('is_to_all' => false, 'group_id' => $groupid));
    	}
    	$mpnews = json_encode(array('media_id' => $mediaid));
    	$xml = sprintf('{
    			"filter" : %s,
    			...
    			$mpnews,
    			"mpnews");
    	$url = self::API_DOMAIN . 'cgi-bin/message/mass/sendall?access_token=' . $this->get_access_token();
    	$res = HttpCurl::post($url, $xml, 'json');
    	// 异常处理: 获取时网络错误
    	if ($res === false) {
    		return Error::code('ERR_GET');
    ...
    public function upload_media($type, $path)
    {
        ...
        // 判断是否调用成功
        if (isset($res->media_id) || isset($res->thumb_media_id)) {
            return array(null, $res);
        } else {
            return array($res, null);
        }
